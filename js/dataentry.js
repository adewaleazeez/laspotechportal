...
var list_obj=null;
var myCheckboxes=0;
function doCheckboxes(id){
	var option="";
	if(id!=null && id!=""){
		var selectoption=document.getElementById(id);
		option=selectoption.options[selectoption.selectedIndex].text;
	}
	if(option!="" && option!="Select/Unselect All"){
		var selectoption=document.getElementById(id);
		var option=selectoption.options[selectoption.selectedIndex].text;
		var option_split = option.split(' - ');
		var first = parseInt(option_split[0],10)-1;
		var last = parseInt(option_split[1],10);
...
function showUpdateMatric(){
	document.getElementById("oldregNumber").value=document.getElementById("regNumber").value;
	if(document.getElementById("oldregNumber").value==null || document.getElementById("oldregNumber").value==""){
        var error = "<br><b>Please correct the following error:</b><br><br><br>Select a Student with Matric No";
        document.getElementById("showError").innerHTML = error;
        $('#showError').dialog('open');
		return true;
	}
	$('#updatematricno').dialog('open');
}

function updateMatricNo(){
	var oldregnumber=document.getElementById("oldregNumber").value;
	var newregnumber=document.getElementById("newregNumber").value;
	var error="";
    if (newregnumber=="") error += "New Reg. No must not be blank.<br><br>";
    if(error.length >0) {
    ...
    var param = "&param="+oldregnumber+"]["+newregnumber;
    var url = "/laspotechportal/dataentrybackend.php?option=updateMatricNo"+param;
    AjaxFunctionDataEntry(url);
}

function deleteStudents(){
	checkAccess('deletestudent', 'Delete Students');
}

function deleteStudent(){
	faculty = document.getElementById("facultycodes").value;
	department = document.getElementById("departmentcodes").value;
	programme = document.getElementById("programmecodes").value;
	level = document.getElementById("studentlevels").value;
	sesions = document.getElementById("sesionsA").value;
	semester = document.getElementById("semestersA").value;
    var error="";
	...
	createCookie("_facultys_",faculty,365);
	createCookie("_departments_",department,365);
	createCookie("_programmes_",programme,365);
	createCookie("_levels_",level,365);
	createCookie("_sessions_",sesions,365);
	createCookie("_semesters_",semester,365);

	var ans = confirm("Are you sure you want to delete all students and marks for "+faculty+", "+department+" department, "+programme+", "+level+" "+sesions+" session, "+semester+" semester ?");
	if(ans){
		document.getElementById("showAlert").innerHTML = "<b>Deleting Records!!!</b><br><br>Wait while records are being deleted";
		$('#showAlert').dialog('open');

		var param='&param='+faculty+']['+department+']['+programme+']['+level+']['+sesions+']['+semester;
		var url = "/laspotechportal/dataentrybackend.php?option=deleteStudents"+param;
		AjaxFunctionDataEntry(url);
	}
}

function showOnlineUpload(){
	$('#uploadresult').dialog('close');
	$('#onlineupload').dialog('open');
}

function onlineUpload(){
	var session=document.getElementById("sesionsA").value;
	var semester=document.getElementById("semesterA").value;
	var error="";
    if (session=="") error += "Session must not be blank.<br><br>";
    if (semester=="") error += "Semester must not be blank.<br><br>";
    if(error.length >0) {
        error = "<br><b>Please correct the following errors:</b> <br><br><br>" + error
        document.getElementById("showError").innerHTML = error;
    ...
    document.getElementById("showAlert").innerHTML = "<b>Please wait...</b><br><br>Online Upload in progress..........";
    $('#showAlert').dialog('open');
    var param = "&param="+session+"]["+semester;
    var url = "/laspotechportal/dataentrybackend.php?option=onlineUpload"+param;
    AjaxFunctionDataEntry(url);
}

function generatePins(){
	var session=document.getElementById("sesions4").value;
	var semester=document.getElementById("semester4").value;
	var error="";
    if (session=="") error += "Session must not be blank.<br><br>";
    if (semester=="") error += "Semester must not be blank.<br><br>";
    if(error.length >0) {
    ...
    document.getElementById("showAlert").innerHTML = "<b>Please wait...</b><br><br>Pin Numbers Generation in progress..........";
    $('#showAlert').dialog('open');
    var param = "&param="+session+"]["+semester;
    var url = "/laspotechportal/dataentrybackend.php?option=generatePins"+param;
    AjaxFunctionDataEntry(url);
}
...
function showDate(){
	displayDatePicker('awardate', false, 'dmy', '/')
}
...
function showAwarDate(){
	...
	if(finalyear=="Yes"){
		...
		document.getElementById('awardatecover').innerHTML="<b>Award Date:&nbsp;<input type='text' style='display:inline' id='awardate'  name='awardate' size='10'  onclick=showDate() />&nbsp;&nbsp;&nbsp;</b>";
	}else{
		document.getElementById('awardatecover').style.visibility = 'hidden';
		document.getElementById('awardatecover').innerHTML="";
	}
}
...
function updateSignatories(option, table){
    ...
    var url = "/laspotechportal/dataentrybackend.php?option="+option+"&table="+table+param+"&currentuser="+readCookie("currentuser")+"&serialno="+serialno;
    AjaxFunctionDataEntry(url);
}
...
function updateGeneral(id){
	...
	//if(selectedoption.match("All")){
		if (facultys=="") error += "School must not be blank.<br><br>";
	...
	//}else 
	if(selectedoption.match("Selected")){
		var selectoption=document.getElementById('selectall');
		var option=selectoption.options[selectoption.selectedIndex].text;

		if(myCheckboxes>200 && option=="Select/Unselect All"){
			for(var i=0; i < myCheckboxes; i++){
				var checkboxid="box"+(i);
				document.getElementById(checkboxid).checked=false;
			}
			document.getElementById("showPrompt").innerHTML = "<b>Too Many Students!!!<b><br><br>You selected too many students, please use the range options.";
	...
	if(id=="selectregister"){
		selectregister=document.getElementById(id);
		selectregister=selectregister.options[selectregister.selectedIndex].text;
		arg += "&codeid=register]["+selectregister
	}
	arg = "?option=updateGeneral&menuoption="+selectedoption+arg
    var url = "/laspotechportal/dataentrybackend.php"+arg+param;
	document.getElementById("showAlert").innerHTML = "<b>Session and Semester Update in Progress!!!</b><br><br><br>Please wait while Session, Semester and Level are being updated.............";
	$('#showAlert').dialog('open');
    AjaxFunctionDataEntry(url);
}
...
/*function checkAccess(access, menuoption, param){
    ...
    var url = "/laspotechportal/dataentrybackend.php?option=checkAccess"+arg;
    AjaxFunctionDataEntry(url);
}*/
...
function viewMyResult(repotype,mode){
    var error="";
	var param ="";
	var selectoption="";
	var facultycode="";
	...
	if(repotype=="viewhtmlmaster" || repotype=="viewexcelmaster" || repotype=="viewwordmaster" || repotype=="viewpdfmaster" || repotype=="graph"){
	    ...
	    resultype=document.getElementById("resultype").value;
		createCookie("_facultys_",facultycode,365);
		createCookie("_departments_",departmentcode,365);
		createCookie("_programmes_",programmecode,365);
		createCookie("_levels_",studentlevel,365);
		createCookie("_sessions_",sessions,365);
		createCookie("_semesters_",semester,365);
	}
	...
	if(repotype=="emailtranscript" || repotype=="viewpdftranscript"){
	    ...
	    sessions = document.getElementById("sesions4A").value;
		semester = document.getElementById("sesions4B").value;
	    selectoption=document.getElementById("finalyear1");
		finalyear=selectoption.options[selectoption.selectedIndex].text;
	    if(finalyear=="Yes") awardate=document.getElementById("awardate").value;
		matricno="";
		var counter=0;
		for(var i=0; i < myCheckboxes; i++){
			var checkboxid="box"+(i);
		...
		if(counter>200){
			...
			error += "You selected too many students, please use the range options.";
		}
		if(matricno.length==0) error += "Please select some students to generate transcripts.<br><br>";
		...
		param2 = "&awardate="+awardate;
		createCookie("_facultys_",facultycode,365);
		createCookie("_departments_",departmentcode,365);
		createCookie("_programmes_",programmecode,365);
		createCookie("_levels_",studentlevel,365);
		createCookie("_sessionsA_",sessions,365);
		//createCookie("_sessionsB_",semester,365);
	}
	...
	if(repotype=="excelexport"){
		...
		studentlevel = document.getElementById("studentlevel5").value;
	...
	if(repotype=="excelexportregular"){
		facultycode = document.getElementById("facultycodes").value;
		departmentcode = document.getElementById("departmentcodes").value;
		programmecode = document.getElementById("programmecodes").value;
		studentlevel = document.getElementById("studentlevels").value;
	    sessions = document.getElementById("sesionsA").value;
		semester = document.getElementById("semestersA").value;
		createCookie("_facultys_",facultycode,365);
		createCookie("_departments_",departmentcode,365);
		createCookie("_programmes_",programmecode,365);
	...
	if(repotype=="excelbackup"){
	    ...
	    sessions = document.getElementById("sesions").value;
		semester = document.getElementById("semester").value;
		createCookie('_faculty',facultycode,false);
		createCookie('_department',departmentcode,false);
		createCookie('_programme',programmecode,false);
		createCookie('_studentlevel',studentlevel,false);
		createCookie('_sesions',sessions,false);
		createCookie('_semester',semester,false);
	}

	if (facultycode=="") error += "School must not be blank.<br><br>";
	if (departmentcode=="") error += "Department must not be blank.<br><br>";
	if (programmecode=="") error += "Programme must not be blank.<br><br>";
    ...
    if(error.length >0 && repotype!="exceluploadreport") {
        error = "<br><b>Please correct the following errors:</b> <br><br><br>" + error;
        document.getElementById("showError").innerHTML = error;
        $('#showError').dialog('open');
        return true;
	...
	//createCookie("_groupcode_",groupsession,365);
	
	var groupsession="";
	param = "?sessions="+sessions+"&semester="+semester+"&facultycode="+facultycode+"&departmentcode="+departmentcode;
	param +="&programmecode="+programmecode+"&studentlevel="+studentlevel+"&finalyear="+finalyear+"&matricno="+matricno+"&groupsession="+groupsession+"&mode="+mode;
	param +="&resultype="+resultype+param2; //+"&leftsigid="+leftsigid+"&midsigid="+midsigid+"&rightsigid="+rightsigid+"&leftsigname="+leftsigname+"&midsigname="+midsigname+"&rightsigname="+rightsigname;
	...
	if(repotype=="viewhtmlmaster"){
    ...
    }else if(repotype=="graph"){
        oWin = window.open("statisticalgraphreport.php"+param, "_blank", "directories=0,scrollbars=1,resizable=1,location=0,status=0,toolbar=0,menubar=0,width=800,height=500,left=100,top=100");
        ...
        $('#showAlert').dialog('open');
		var url = "/laspotechportal/mailtranscript.php"+param;
		AjaxFunctionDataEntry(url);
        return true;
	...
	}else if(repotype=="excelexportregular"){
		oWin = window.open("excelexportregular.php"+param, "_blank", "directories=0,scrollbars=1,resizable=1,location=0,status=0,toolbar=0,menubar=0,width=800,height=500,left=100,top=100");
	}else if(repotype=="excelbackup"){
		oWin = window.open("excelbackup.php"+param, "_blank", "directories=0,scrollbars=1,resizable=1,location=0,status=0,toolbar=0,menubar=0,width=800,height=500,left=100,top=100");
	}else if(repotype=="exceluploadreport"){
		oWin = window.open("uploaderror.php", "_blank", "directories=0,scrollbars=1,resizable=1,location=0,status=0,toolbar=0,menubar=0,width=800,height=500,left=100,top=100");
	}
...
function allocatePins(){
	...
	var programmecode = document.getElementById("programmecode4").value;
	var studentlevel = document.getElementById("studentlevel4").value;
	var sessions = document.getElementById("sesions4").value;
	var semester = document.getElementById("semester4").value;
	...
	if(matricno.length==0) error += "Please select some students to allocate pin numbers to.<br><br>";
	if (facultycode=="") error += "Faculty must not be blank.<br><br>";
	if (departmentcode=="") error += "Department must not be blank.<br><br>";
	if (programmecode=="") error += "Programme must not be blank.<br><br>";
	if (studentlevel=="") error += "Student level must not be blank.<br><br>";
	...
	var param = "&sesions="+sessions+"&semester="+semester+"&facultycode="+facultycode+"&departmentcode="+departmentcode;
	param +="&programmecode="+programmecode+"&studentlevel="+studentlevel+"&matricno="+matricno;
    var url = "/laspotechportal/dataentrybackend.php?option=allocatePins"+param;
	AjaxFunctionDataEntry(url);
}
...
function sendSMS(){
	...
	var senderid = "LASPO_SPTS"; //document.getElementById("senderid").value;
	...
	smsmessage = encodeURIComponent(smsmessage).replace(/!/g, '%21').replace(/'/g, '%27').replace(/\(/g, '%28').replace(/\)/g, '%29').replace(/\*/g, '%2A').replace(/%20/g, '+');
    ...
    //var url = "/laspotechportal/dataentrybackend.php?option=approveMark"+param;
    var url = "/laspotechportal/dataentrybackend.php?option=sendSms&phone="+recipients+"&senderid="+senderid;
...
function reverseMarks(){
    ...
    var url = "/laspotechportal/dataentrybackend.php?option=reverseMark"+param;
    AjaxFunctionDataEntry(url);
}
...
function checkPin(){
    ...
    var url = "/laspotechportal/dataentrybackend.php?option=checkPin"+"&pinno="+pinno+"&currentUser="+currentuser;
    AjaxFunctionDataEntry(url);
}
...
/*function checkCode(id,table,codeid){
	...
	if(codevalue!=null && codevalue!=""){
		var url = "/laspotechportal/dataentrybackend.php?option=checkCode"+"&table="+table+"&codeid="+codeid+"&codevalue="+codevalue+"&access="+id;
		AjaxFunctionDataEntry(url);
	}
}*/
...
function checkReceipt(){
    ...
    var currentuser = readCookie("currentuser");
    var url = "/laspotechportal/dataentrybackend.php?option=checkReceipt"+"&receiptno="+receiptno+"&confirmno="+confirmno+"&currentUser="+currentuser;
    AjaxFunctionDataEntry(url);
}
...
function addDoc(){
    ...
    for(var k=1; k<docrow; k++){
        ...
        if(docdescvalue.length>0){
            str += "<td><div id='"+actionid+"'><a href=javascript:viewDoc('"+docid.replace(/ /g, '#')+"')>View</a>&nbsp;&nbsp;<a href=javascript:deleteDoc('"+k+"')>Delete</a></div></td>";
        }else{
...
function populateDoc(arg){
    ...
    for(var k=1; k<row_split.length; k++){
        ...
        str += "<input type='hidden' value='"+docidvalue+"' id='"+docid+"' /></td>";
        if(docdescvalue.length>0){
            str += "<td><div id='"+actionid+"'><a href=javascript:viewDoc('"+docidvalue.replace(/ /g, '#')+"')>View</a>&nbsp;&nbsp;<a href=javascript:deleteDoc('"+k+"')>Delete</a></div></td>";
        }else{
            str += "<td><div id='"+actionid+"'><a href=javascript:uploadDoc('"+k+"')>Upload new document</a></div></td>";
        }
    }
...
function populateStudentHistory(arg,regno){
    var row_split = arg.split('_~_');
	var str = "";
	var docs = "";
    var col_split = "";
	...
	for(var k=1; k<=row_split.length-1; k++){
		...
		if(col_split[1]!=null && col_split[1]!=""){
			var id = "student_history_msg"+k;
			var upd = "u_~_"+col_split[1]+"_~_"+regno+"_~_"+id;
			var del = "d_~_"+col_split[1]+"_~_"+regno+"_~_"+id;
			str += "<table style='font-size:11px' align='center' class='history_message' cellspacing='0' cellpadding='1' width='100%' border=0>";
			assignor += d.getDate()+" "+d.getFullYear()+" "+hour+":"+min+":"+sec;
			assignor += " - "+col_split[0];

			str +="<tr><th colspan='4'><div style='font-size:12px; font-weight:bold'>"+assignor+"</div></th></tr>";
			str +="<tr><td><textarea id='"+id+"' style='display:inline;' rows='5' cols='100'>"+col_split[2]+"</textarea><br>";
            str += "<input id='"+upd+"' type='button' style='display:inline' onclick=updateHistory(this.id); value=' UPDATE ' />&nbsp;&nbsp;";
            str += "<input id='"+del+"' type='button' style='display:inline' onclick=deleteHistory(this.id); value=' DELETE ' /></td></tr></table><br>";
			//str +="<div id='"+iddelete+"' onclick=deleteHistory('"+col_split[1]+"','"+regno+"','"+id+"'); style='display:block; display:inline; margin-left: 15px; width: 105px; padding: 5px 5px; text-align:center; background:#C0C0C0; border-bottom:1px solid #ddd;color:#000000; cursor: pointer; border:1px solid #000033; height: 12px; width: 50px; font-family: Geneva, Arial, Helvetica, sans-serif; font-size: 12px; font-weight: bold;'>Delete</div></td></tr></table><br>";
		}
    }//'"+col_split[1]+"','"+regno+"','"+id+"'
    document.getElementById('student_history').innerHTML=str+"<br>";
	var d = new Date();
	...
	assignor += d.getDate() + " " + d.getFullYear() + " " + hour + ":" + min + ":" + sec;
	assignor += " - " + readCookie("currentuser");
	document.getElementById("userinfo").innerHTML = "<div style='font-size:12px; font-weight:bold'>"+assignor+"</div>";
	var thedate = year + "-" + mon + "-" + day + " " + hour + ":" + min + ":" + sec;
	document.getElementById("student_history_msg").focus();
}

function saveHistory(regno){
	var msg = document.getElementById("student_history_msg").value;

    if(msg.length==0) {
        error = "<br><b>Please correct the following errors:</b> <br><br><br>Message must not be blank";
        document.getElementById("showError").innerHTML = error;
        $('#showError').dialog('open');
        return true;
    }
	...
	document.getElementById("showAlert").innerHTML = "<b>Please wait...</b><br><br>Processing your record.";
    $('#showAlert').dialog('open');
	document.getElementById("student_history_msg").value="";
	var param = "&param="+readCookie("currentuser")+"]["+date+"]["+msg+"]["+regno+"][SAVE";
    var url = "/laspotechportal/dataentrybackend.php?option=updateStudentHistory"+param;
	AjaxFunctionDataEntry(url);
}

function updateHistory(arg){
	var cols = arg.split('_~_');
	var date = cols[1];
	var regno = cols[2];
	var id = cols[3];
	...
	document.getElementById("showAlert").innerHTML = "<b>Please wait...</b><br><br>Processing your record.";
    $('#showAlert').dialog('open');
	var param = "&param="+readCookie("currentuser")+"]["+date+"]["+msg+"]["+regno+"][UPDATE";
    var url = "/laspotechportal/dataentrybackend.php?option=updateStudentHistory"+param;
	AjaxFunctionDataEntry(url);
}
...
function deleteHistory(arg){
    ...
    $('#showAlert').dialog('open');
	var msg = document.getElementById(id).value;
	var param = "&param="+readCookie("currentuser")+"]["+date+"]["+msg+"]["+regno+"][DELETE";
    var url = "/laspotechportal/dataentrybackend.php?option=updateStudentHistory"+param;
	AjaxFunctionDataEntry(url);
}
...
function updateRegularStudent(option, table){
    ...
    var carryover = document.getElementById("carryover").value;
    var sesionsreabsurption = document.getElementById("sesionsreabsurption").value;
	...
	var entryyear="";
	param += "]["+lockrec+"]["+qualification+"]["+minimumunit+"]["+tcp+"]["+tnu+"]["+gpa+"]["+tnup+"]["+entryyear+"]["+phoneno+"]["+guardianphoneno+"]["+admissiontype+"]["+carryover+"]["+sesionsreabsurption;
	var param2 = "&param2="+regNumber+"]["+studentlevel+"]["+sessionss+"]["+semesterss;
    var url = "/laspotechportal/dataentrybackend.php?option="+option+"&table="+table+param+param2+"&currentuser="+readCookie("currentuser")+"&serialno="+serialno;
	AjaxFunctionDataEntry(url);
}

function updateCourseStatus(statusid,matno,camarkid,examarkid,markid){
	...
	if (cstatus!=""){
		document.getElementById(camarkid).value="";
		if(cstatus=="Sick")	document.getElementById(examarkid).value="S";
		if(cstatus=="Absent")	document.getElementById(examarkid).value="ABS";
		if(cstatus=="Did Not Register")	document.getElementById(examarkid).value="DNR";
		if(cstatus=="Incomplete")	document.getElementById(examarkid).value="I";
		document.getElementById(markid).value="";
	}else{
		document.getElementById(camarkid).value="";
		document.getElementById(examarkid).value="";
		document.getElementById(markid).value="";
	}
    camark = document.getElementById(camarkid).value;
    examark = document.getElementById(examarkid).value;
	...
	var entryyear="";
	param += "]["+mark+"]["+obtainable+"]["+percentage+"]["+studentlevel+"]["+programme+"]["+faculty+"]["+department+"]["+entryyear+"]["+camark+"]["+examark;
	var option = "addRecord";
	if(cstatus==null || cstatus=="") option = "deleteRecord";
    var url = "/laspotechportal/dataentrybackend.php?option="+option+"&table=retakecourses"+param+param2+"&currentuser="+readCookie("currentuser")+"&serialno="+serialno;
	AjaxFunctionDataEntry(url);
}
...
function doMaxUnit(arg){
    ...
    var department = document.getElementById("departmentcode5").value;
    var programme = document.getElementById("programmecode5").value;
    var level = document.getElementById("studentlevel5").value;
    var sesions = document.getElementById("sesions").value;
    var semester = document.getElementById("semester").value;
	...
	createCookie("_facultys",faculty,365);
	createCookie("_departments",department,365);
	createCookie("_programmes",programme,365);
	createCookie("_levels",level,365);
	createCookie("_sessions",sesions,365);
	createCookie("_semesters",semester,365);
...
function populateMark(arg, sortorder){
	...
	createCookie("_facultys",faculty,365);
	createCookie("_departments",department,365);
	createCookie("_programmes",programmecode,365);
	createCookie("_levels",studentlevel,365);
	createCookie("_sessions",sesions,365);
	createCookie("_semesters",semester,365);
	//createCookie("_entryyears",entryyear,365);
	createCookie("_coursecodes",coursecode,365);
	var entryyear="";
	...
	param += "&programmecode="+programmecode+"&facultycode="+faculty+"&departmentcode="+department+"&sortorder="+sortorder;
    var url = "/laspotechportal/dataentrybackend.php?option=getAllRecs"+param;
    //popmark = "1";
...
function deleteMarks(){
	...
	var ans = confirm("Are you sure you want to delete all marks  for "+coursecode);
	if(ans){
		var param = "&table=examresultstable&sesions="+sesions+"&semester="+semester;
		param += "&coursecode="+coursecode+"&markdescription="+markdescription+"&studentlevel="+studentlevel;
		param += "&programmecode="+programme+"&facultycode="+faculty+"&departmentcode="+department;
		var url = "/laspotechportal/dataentrybackend.php?option=deleteMarks"+param;
		AjaxFunctionDataEntry(url);
	}
}
...
function populateMarks(arg){
	...
	str += "<td>CA_Score</td><td>Exam_Score</td><td>Total_Score</td><td>Retake_Status</td></tr>";
	...
	for(var k=1; k<row_split.length-1; k++){
        ...
        //marksrow++;
        matnovalue = col_split[15];
        camarkvalue = col_split[21];
        examarkvalue = col_split[22];
		if(col_split[20]=="Sick"){
			examarkvalue = "S";
		}else if(col_split[20]=="Absent"){
			examarkvalue = "ABS";
		}else if(col_split[20]=="Did Not Register"){
			examarkvalue = "DNR";
		}else if(col_split[20]=="Incomplete"){
			examarkvalue = "I";
		}
        markvalue = col_split[6];
        serialnovalue = col_split[0];
		var fullname = col_split[16].toUpperCase()+", "+col_split[17];
		if(col_split[18]>"") fullname += " "+col_split[18];
        ...
        str += "<td width='10%'><input type='text' readonly disabled='true' value='"+camarkvalue+"' id='"+camarkid+"' size='10' /></td>";
        str += "<td width='10%'><input type='text' readonly disabled='true' value='"+examarkvalue+"' id='"+examarkid+"' size='10' /></td>";
        str += "<td width='10%'><input type='text' readonly disabled='true' value='"+markvalue+"' id='"+markid+"' onblur=updateMark('"+serialnovalue+"','"+matnovalue+"','"+camarkid+"','"+examarkid+"','"+markid+"','addRecord','"+statusid+"') size='10' /></td>";
		if(col_split[20]==null || col_split[20]==""){
			str += "<td><select id='"+statusid+"' readonly disabled='true' onchange=updateCourseStatus('"+statusid+"','"+col_split[15]+"','"+camarkid+"','"+examarkid+"','"+markid+"')><option selected></option><option>Sick</option><option>Absent</option><option>Did Not Register</option><option>Incomplete</option></select></td></tr>"; //<option>Retake</option>
		}else if(col_split[20]=="Sick"){
			str += "<td><select id='"+statusid+"' readonly disabled='true' onchange=updateCourseStatus('"+statusid+"','"+col_split[15]+"','"+camarkid+"','"+examarkid+"','"+markid+"')><option></option><option selected>Sick</option><option>Absent</option><option>Did Not Register</option><option>Incomplete</option></select></td></tr>"; //<option>Retake</option>
		}else if(col_split[20]=="Absent"){
			str += "<td><select id='"+statusid+"' readonly disabled='true' onchange=updateCourseStatus('"+statusid+"','"+col_split[15]+"','"+camarkid+"','"+examarkid+"','"+markid+"')><option></option><option>Sick</option><option selected>Absent</option><option>Did Not Register</option><option>Incomplete</option></select></td></tr>"; //<option>Retake</option>
		}else if(col_split[20]=="Did Not Register"){
			str += "<td><select id='"+statusid+"' readonly disabled='true' onchange=updateCourseStatus('"+statusid+"','"+col_split[15]+"','"+camarkid+"','"+examarkid+"','"+markid+"')><option></option><option>Sick</option><option>Absent</option><option selected>Did Not Register</option><option>Incomplete</option></select></td></tr>"; //<option>Retake</option>
		}else if(col_split[20]=="Incomplete"){
			str += "<td><select id='"+statusid+"' readonly disabled='true' onchange=updateCourseStatus('"+statusid+"','"+col_split[15]+"','"+camarkid+"','"+examarkid+"','"+markid+"')><option></option><option>Sick</option><option>Absent</option><option>Did Not Register</option><option selected>Incomplete</option></select></td></tr>"; //<option>Retake</option>
		}
        //str += "<td><a href=javascript:updateMark('"+serialnoid+"','"+matnoid+"','"+markid+"','updateRecord')>Update</a>&nbsp;&nbsp;<a href=javascript:updateMark('"+serialnoid+"','"+matnoid+"','"+markid+"','deleteRecord')>Delete</a></td></tr>";
    }
    str += "</table>";
...
function updateMark(serialno,matno,camarkid,examarkid,markid,option,statusid){
    ...
    if (isNaN(percentage)) error += "Percentage Overall must be numeric.<br><br>";
    if (isNaN(obtainable)) error += "Marks Obtainable must be numeric.<br><br>";
    if (isNaN(camark)) error += "CA Score must be numeric.<br><br>";
    if (isNaN(examark)) error += "Exam Score must be numeric.<br><br>";
    if (isNaN(mark)) error += "Total Score must be numeric.<br><br>";
	marks=mark;
	if (mark=="") marks=0;
	camarks=camark;
	if (camark=="") camarks=0;
	examarks=examark;
	if (examark=="") examarks=0;
	if( parseFloat(marks) != (parseFloat(camarks) + parseFloat(examarks)) ){
		error += "CA and Exam Scores do not add up to the Total Score.<br><br>";
		document.getElementById(camarkid).value="";
		document.getElementById(examarkid).value="";
		document.getElementById(markid).value="";
	}
...
function updateAmendMark(serialno,matno,markid,newmarkid,option,reasonid){
    ...
    var amendedtitle = document.getElementById("amendedtitle").value;
    var amendreason = document.getElementById("amendreason").value;
    ...
    //if (entryyear=="") error += "Group Code must not be blank.<br><br>";
    if (coursecode=="") error += "Course Code must not be blank.<br><br>";
    if (amendedtitle=="") error += "Title for Amendment must not be blank.<br><br>";
    if (amendreason=="") error += "Reason for Amendment must not be blank.<br><br>";
    if (markdescription=="") error += "Marks Description must not be blank.<br><br>";
    if (percentage=="") error += "Percentage Overall must not be blank.<br><br>";
    if (obtainable=="") error += "Marks Obtainable must not be blank.<br><br>";
    if (matno=="") error += "Matric number must not be blank.<br><br>";
	...
	var param2 = "&param2="+faculty+"]["+department+"]["+programme+"]["+entryyear+"]["+studentlevel+"]["+sesions+"]["+semester+"]["+coursecode+"]["+amendedtitle+"]["+amendreason;
    var url = "/laspotechportal/dataentrybackend.php?option="+option+"&table="+table+param+"&currentuser="+readCookie("currentuser")+"&serialno="+serialno+param2;
	AjaxFunctionDataEntry(url);
}
...
function populateAmended(arg,sortorder){
	if(sortorder==null) sortorder="";
    var faculty = document.getElementById("facultycode1").value;
    var department = document.getElementById("departmentcode1").value;
    ...
    //if (entryyear=="") error += "Group Code must not be blank.<br><br>";
    if (markdescription=="") error += "Marks Description must not be blank.<br><br>";
    if (amendedtitle=="") error += "Amendment Title must not be blank.<br><br>";
    if(error.length >0) {
        error = "<br><b>Please correct the following errors:</b> <br><br><br>" + error;
        document.getElementById("showError").innerHTML = error;
        $('#showError').dialog('open');
	...
	createCookie("_semesters",semester,365);
	//createCookie("_entryyears",entryyear,365);
	createCookie("_amendedtitles",amendedtitle,365);
	createCookie("_coursecodes",coursecode,365);
	
	var entryyear="";
    var param = "&table=amendedresults&sesions="+sesions+"&semester="+semester+"&entryyear="+entryyear+"&access="+arg;
    param += "&coursecode="+coursecode+"&markdescription="+markdescription+"&amendedtitle="+amendedtitle+"&studentlevel="+studentlevel;
	param += "&programmecode="+programmecode+"&facultycode="+faculty+"&departmentcode="+department;
    var url = "/laspotechportal/dataentrybackend.php?option=getAllRecs"+param+"&sortorder="+sortorder;
	if(sortorder==null) sortorder="";
...
function populateAmendeds(arg){
	...
	str += "<td>Previous_Marks</td><td>Amended_Marks</td></tr>";
	//<td>Amendment_Reasons</td>
	//<a href=javascript:populateMark('b.marksobtained') onmouseover=toolTip('msgdiv','Click_here_to_sort_by_Marks',event) onmouseout=toolTip('msgdiv','',event)></a>
    var flag=0;
    var matnoid="";
    var markid="";
	...
	for(var k=1; k<row_split.length-1; k++){
        ...
        serialnovalue = col_split[0];
		var fullname = col_split[24].toUpperCase()+", "+col_split[25];
		if(col_split[26]>"") fullname += " "+col_split[26];
        ...
        str += "<td><input type='text' value='"+newmarkvalue+"' id='"+newmarkid+"' size='10' onblur=updateAmendMark('"+serialnovalue+"','"+matnovalue+"','"+markid+"','"+newmarkid+"','addRecord','') /></td>";
		...
		/*if(col_split[19]==null || col_split[19]==""){
			str += "<td><select id='"+reasonid+"' onchange=updateCourseStatus('"+reasonid+"','"+col_split[14]+"','"+markid+"')><option selected></option><option>Absent</option><option>Ignore PCE</option><option>Retake</option></select></td></tr>";
		}else if(col_split[19]=="Absent"){
			str += "<td><select id='"+reasonid+"' onchange=updateCourseStatus('"+reasonid+"','"+col_split[14]+"','"+markid+"')><option></option><option selected>Absent</option><option>Ignore PCE</option><option>Retake</option></select></td></tr>";
		}else if(col_split[19]=="Ignore PCE"){
			str += "<td><select id='"+reasonid+"' onchange=updateCourseStatus('"+reasonid+"','"+col_split[14]+"','"+markid+"')><option></option><option>Absent</option><option selected>Ignore PCE</option><option>Retake</option></select></td></tr>";
		}else if(col_split[19]=="Retake"){
			str += "<td><select id='"+reasonid+"' onchange=updateCourseStatus('"+reasonid+"','"+col_split[14]+"','"+markid+"')><option></option><option>Absent</option><option>Ignore PCE</option><option selected>Retake</option></select></td></tr>";
		}*/
        //str += "<td><a href=javascript:updateMark('"+serialnoid+"','"+matnoid+"','"+markid+"','updateRecord')>Update</a>&nbsp;&nbsp;<a href=javascript:updateMark('"+serialnoid+"','"+matnoid+"','"+markid+"','deleteRecord')>Delete</a></td></tr>";
    }
    str += "</table>";
...
function getAmendReason(){
	...
	var entryyear="";
    var param = "&table=amendedreasons&sesions="+sesions+"&semester="+semester+"&entryyear="+entryyear;
    param += "&coursecode="+coursecode+"&amendedtitle="+amendedtitle+"&studentlevel="+studentlevel;
	param += "&programmecode="+programmecode+"&facultycode="+faculty+"&departmentcode="+department;
    var url = "/laspotechportal/dataentrybackend.php?option=getARecord"+param;
	AjaxFunctionDataEntry(url);
}
...
function resetForm(){
    ...
    document.getElementById("student_history_msg").value="";
    document.getElementById("sesionsreabsurption").value="";
...
/*    if(table=='studentfees') {
    ...
    if(table=='listpins') {
		...
		var levels = document.getElementById("studentlevel4").value;
		var sessions = document.getElementById("sesions4").value;
		var semesters = document.getElementById("semester4").value;
		...
		createCookie("_faculty_",facultys,365);
		createCookie("_department_",departments,365);
		createCookie("_programme_",programmes,365);
		createCookie("_studentlevel_",levels,365);
		createCookie("_sessions_",sessions,365);
		createCookie("_semester_",semesters,365);

        param += "&facultycode="+facultys+"&departmentcode="+departments+"&programmecode="+programmes;
        param += "&studentlevel="+levels+"&sesions="+sessions+"&semester="+semesters;
		if(serialno=="pintype") {
			var selectoption=document.getElementById(serialno);
			access=selectoption.options[selectoption.selectedIndex].text;
			param += "&access="+access;
		}
    }
    if(table=='filterbutton') {
		...
		createCookie("_facultys",facultys,365);
		createCookie("_departments",departments,365);
		createCookie("_programmes",programmes,365);
		createCookie("_levels",levels,365);
		createCookie("_sessions",sessions,365);
		createCookie("_semesters",semesters,365);
		//createCookie("_entryyears",entryyears,365);
		createCookie("_selectactive",active,365);
		createCookie("_studentno",studentno,365);
    ...
    if(table=='listrecords') {
        ...
        var programmes = document.getElementById("programmecode4").value;
		var levels = document.getElementById("studentlevel4").value;
		var sessions = document.getElementById("sesions4A").value;
		var semesters = document.getElementById("sesions4B").value;

		createCookie("_facultys_",facultys.replace(/\,/g, '_'),365);
		createCookie("_departments_",departments.replace(/\,/g, '_'),365);
		createCookie("_programmes_",programmes.replace(/\,/g, '_'),365);
		createCookie("_levels_",levels.replace(/\,/g, '_'),365);
		createCookie("_sessions_",sessions.replace(/\,/g, '_'),365);
		createCookie("_semester_",semesters.replace(/\,/g, '_'),365);
    ...
    document.getElementById("showAlert").innerHTML = "<b>Please wait...</b><br><br>Fetching your data.......";
    $('#showAlert').dialog('open');
    var url = "/laspotechportal/dataentrybackend.php?option=getAllRecs"+"&table="+table+"&serialno="+serialno+param;
	AjaxFunctionDataEntry(url);
}
...
function getUser(email){
    ...
    var url = "/laspotechportal/dataentrybackend.php?option=getUser&userEmail="+email;
    AjaxFunctionDataEntry(url);
}
...
function populateRecords(serialno, table, regnum){
    ...
    var url = "/laspotechportal/dataentrybackend.php?option=getARecord"+"&table="+table+"&serialno="+serialno+param;
    AjaxFunctionDataEntry(url);
}
...
function getRecordlist(arg2,arg3,arg4){
	...
	if(arg2=="departmentcodes"){
		...
		if(facultycode==null || facultycode==""){
			...
			document.getElementById("facultycodes").focus();
			return true;
		}
	...
	if(arg2=="programmecodes"){
		...
		if(departmentcode==null || departmentcode==""){
			...
			document.getElementById("departmentcodes").focus();
			return true;
		}
		createCookie("parent_obj",departmentcode,false);
	}
	if(arg2=="studentlevels"){
		var programmecode=document.getElementById("programmecodes").value;
		if(programmecode==null || programmecode==""){
			alert("programme selection is dependent on Programme selected.\nPlease select a programme");
			document.getElementById("programmecodes").focus();
			return true;
		}
	...
	if(arg2=="departmentcode1"){
		...
		if(facultycode==null || facultycode==""){
			alert("Department selection is dependent on School selected.\nPlease select a faculty");
			document.getElementById("facultycode1").focus();
			return true;
		}
		createCookie("parent_obj",facultycode,false);
	}
	if(arg2=="programmecode1"){
		...
		if(departmentcode==null || departmentcode==""){
			alert("programme selection is dependent on Department selected.\nPlease select a department");
			document.getElementById("departmentcode1").focus();
			return true;
		}
		createCookie("parent_obj",departmentcode,false);
	}
	...
	if(arg2=="departmentcode3"){
		...
		if(facultycode==null || facultycode==""){
			alert("Department selection is dependent on School selected.\nPlease select a faculty");
			document.getElementById("facultycode3").focus();
			return true;
		}
		createCookie("parent_obj",facultycode,false);
	}
	if(arg2=="programmecode3"){
		...
		if(departmentcode==null || departmentcode==""){
			alert("programme selection is dependent on Department selected.\nPlease select a department");
			document.getElementById("departmentcode3").focus();
			return true;
		}
		createCookie("parent_obj",departmentcode,false);
	}
	if(arg2=="studentlevel3"){
		var programmecode=document.getElementById("programmecode3").value;
		if(programmecode==null || programmecode==""){
			alert("programme selection is dependent on Programme selected.\nPlease select a programme");
			document.getElementById("programmecode3").focus();
			return true;
		}
		createCookie("parent_obj",programmecode,false);
	}
	if(arg2=="departmentcode4"){
		...
		if(facultycode==null || facultycode==""){
			alert("Department selection is dependent on School selected.\nPlease select a faculty");
			document.getElementById("facultycode4").focus();
			return true;
		}
		createCookie("parent_obj",facultycode,false);
	}
	if(arg2=="programmecode4"){
		...
		if(departmentcode==null || departmentcode==""){
			alert("programme selection is dependent on Department selected.\nPlease select a department");
			document.getElementById("departmentcode4").focus();
			return true;
		}
		createCookie("parent_obj",departmentcode,false);
	}
	if(arg2=="studentlevel4"){
		var programmecode=document.getElementById("programmecode4").value;
		if(programmecode==null || programmecode==""){
			alert("programme selection is dependent on Programme selected.\nPlease select a programme");
			document.getElementById("programmecode4").focus();
			return true;
		}
		createCookie("parent_obj",programmecode,false);
	}
	if(arg2=="departmentcode5"){
		...
		if(facultycode==null || facultycode==""){
			alert("Department selection is dependent on School selected.\nPlease select a faculty");
			document.getElementById("facultycode5").focus();
			return true;
		}
		createCookie("parent_obj",facultycode,false);
	}
	if(arg2=="programmecode5"){
		...
		if(departmentcode==null || departmentcode==""){
			alert("programme selection is dependent on Department selected.\nPlease select a department");
			document.getElementById("departmentcode5").focus();
			return true;
		}
		createCookie("parent_obj",departmentcode,false);
	}
	if(arg2=="studentlevel5"){
		var programmecode=document.getElementById("programmecode5").value;
		if(programmecode==null || programmecode==""){
			alert("programme selection is dependent on Programme selected.\nPlease select a programme");
			document.getElementById("programmecode5").focus();
			return true;
		}
		createCookie("parent_obj",programmecode,false);
	}
	if(arg2=="departmentcode6"){
		...
		if(facultycode==null || facultycode==""){
			alert("Department selection is dependent on School selected.\nPlease select a faculty");
			document.getElementById("facultycode6").focus();
			return true;
		}
		createCookie("parent_obj",facultycode,false);
	}
	if(arg2=="programmecode6"){
		...
		if(departmentcode==null || departmentcode==""){
			alert("programme selection is dependent on Department selected.\nPlease select a department");
			document.getElementById("departmentcode6").focus();
			return true;
		}
		createCookie("parent_obj",departmentcode,false);
	}
    ...
    var url = "/laspotechportal/dataentrybackend.php?option=getRecordlist&table="+arg3+"&currentobject="+arg2+param;
	AjaxFunctionDataEntry(url);
}
...
function populateCode(code){
	...
	if(curr_obj.id.substr(0,7)=="faculty") {
		createCookie("facultycookie",curr_obj.value,true);
	}
	if(curr_obj.id.substr(0,7)=="department") {
		createCookie("departmentcookie",curr_obj.value,true);
	}
	if(curr_obj.id.substr(0,7)=="programme") {
		createCookie("programmecookie",curr_obj.value,true);
	}
	if(curr_obj.id.substr(0,7)=="studentlevel") {
		createCookie("studentlevelcookie",curr_obj.value,true);
	}
	if(curr_obj.id.substr(0,7)=="sesion") {
		createCookie("sesioncookie",curr_obj.value,true);
	}
	if(curr_obj.id.substr(0,7)=="semester") {
		createCookie("semestercookie",curr_obj.value,true);
	}
...
function doClick(id,table,recordlist){
	document.getElementById(id).value=''; 
	getRecordlist(id,table,recordlist);
}
...
function AjaxFunctionDataEntry(arg){
    ...
    var break_url = url.split("/laspotechportal");
...
function stateChangedAmins(){
    if (xmlhttp.readyState==4){
        ...
        //if(resp.match("Whatsgwan")){
		//	alert(resp);
		//}

        if(resp.match("checkAccess")){
            if(resp.match("checkAccessSuccess")){
				if(readCookie('access').match('deletestudent')){
					deleteStudent();
				}else{
					window.location="home.php?pgid=1";
				}
        ...
        if(resp.match("deleteMarks")){
			populateMark('a.regNumber','DESC');
		}
...
//return true;
            ...
            var allrecords = "<table border='0' style='border-color:#fff;border-style:solid;border-width:1px;height:10px; width:100%;background-color:#336699;margin-top:1px;font-size:10px;'>";
            allrecords += "<tr style='height:32px; font-weight:bold; color:white'>";
			if(resp.match("Warning")){
				resp = "<b>No Match Found!!!</b><br><br>No record(s) found that match the criteria.";
				document.getElementById("showPrompt").innerHTML = resp;
				$('#showPrompt').dialog('open');
				return true;
			}
			...
			if(break_resp[0]=="examresultstable"){
                populateMarks(resp);
			}else if(break_resp[0]=="uniquereferencenumbers"){
                allrecords += "<td width='5%'>S/No</td><td>Pin Numbers</td><td>Pin Status</td><td>Matric No</td><td>Names</td><td>School</td><td>Departments</td><td>Programmes</td><td>Levels</td></tr>";
                ...
                allrecords += "<td>School:</td>";
                allrecords += "<td><input type='text' id='facultycodes' onkeyup=getRecordlist(this.id,'facultiestable','recordlist') onclick=doClick(this.id,'facultiestable','recordlist') size='50' /></td>";

				allrecords += "<td>Department:</td>";
                allrecords += "<td><input type='text' id='departmentcodes' onkeyup=getRecordlist(this.id,'departmentstable','recordlist') onclick=doClick(this.id,'departmentstable','recordlist') size='50' /></td></tr>";

                allrecords += "<tr style='height:32px; font-weight:bold; color:white'>";
                allrecords += "<td>Programme:</td>";
                allrecords += "<td><input type='text' id='programmecodes' onkeyup=getRecordlist(this.id,'programmestable','recordlist') onclick=doClick(this.id,'programmestable','recordlist') size='50' /></td>";

				allrecords += "<td>Level:</td>";
                allrecords += "<td><input type='text' id='studentlevels' onkeyup=getRecordlist(this.id,'studentslevels','recordlist') onclick=doClick(this.id,'studentslevels','recordlist') size='50' /></td></tr>";

                allrecords += "<tr style='height:32px; font-weight:bold; color:white'>";
                allrecords += "<td>Session:</td>";
                allrecords += "<td><input type='text' id='sesionsA' onkeyup=getRecordlist(this.id,'sessionstable','recordlist') onclick=doClick(this.id,'sessionstable','recordlist') size='50' /></td>";

				allrecords += "<td>Semester:</td>";
                allrecords += "<td><input style='display:inline' type='text' id='semestersA' onkeyup=getRecordlist(this.id,'sessionstable','recordlist') onclick=doClick(this.id,'sessionstable','recordlist') size='50' /></td>";
				
				//allrecords += "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Group Code:&nbsp;&nbsp;<input style='display:inline' type='text' id='entryyearsA' onkeyup=getRecordlist(this.id,'sessionstable','recordlist') onfocus=getRecordlist(this.id,'sessionstable','recordlist') size='10' /></td></tr>";
                ...
                allrecords += "<tr style='height:32px; font-weight:bold; color:white'>";
				//allrecords += "<td>Group Code:</td>";
                //allrecords += "<td><input style='display:inline' type='text' id='entryyearsA' onkeyup=getRecordlist(this.id,'sessionstable','recordlist') onclick=doClick(this.id,'facultiestable','recordlist') size='50' /></td>";
				allrecords += "<td>Names/Email/Matric No:</td>";
                allrecords += "<td><input type='text' id='students' onkeyup=getRecordlist(this.id,'regularstudents','recordlist') onclick=doClick(this.id,'regularstudents','recordlist') size='50' /></td>";
				...
				allrecords += "<tr style='height:32px; font-weight:bold; color:white'>";
				allrecords += "<td></td>";
				allrecords += "<td colspan='3'>";
                allrecords += "<input type='button' style='display:inline' id='filterbutton' onclick=getRecords(this.id) value=' List Records ' />";
                allrecords += "<input type='button' style='display:inline' id='import' onclick=importRecords() value=' Import Records From Excel ' />";
                allrecords += "<input type='button' style='display:inline' id='excelexportregular' onclick='viewMyResult(this.id)' value=' Export Records To Excel ' />";
                allrecords += "<input type='button' style='display:inline' id='deletestudents' onclick='deleteStudents();' value=' Delete All Students ' />";
                allrecords += "</tr></table>";
                allrecords += "<div id='f1_upload_process2' style='z-index:100; visibility:hidden; position:absolute; text-align:center; width:400px; top:75px; left:400px'>Uploading...<br/><img src='imageloader.gif' /><br/></div>";

				allrecords += "<table border='0' style='border-color:#fff;border-style:solid;border-width:1px; height:10px;width:100%;background-color:#336699;margin-top:5px;font-size:10px;'>";
                allrecords += "<tr style='font-weight:bold; color:white'>";
                allrecords += "<td width='5%'>S/No</td><td width='10%'>Matric_No</td><td width='10%'>Names</td><td width='10%'>School</td>";
                allrecords += "<td width='10%'>Department</td><td width='10%'>Programme</td><td width='10%'>Session</td>";
				allrecords += "<td width='10%'>Semester</td><td width='10%'>Level</td><td align='center' width='10%'>Active</td><td align='center' width='10%'>Lock Result</td><td align='center' width='10%'>Registered</td><td width='10%'>Select All</td></tr>";
				...
				allrecords += "<td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>";
				...
				allrecords += "<td><Select id='selectregister' onchange='updateGeneral(this.id);'><option>--Select Options--</option><option>Register Selected</option><option>Deregister Selected</option></select></td>";
				//allrecords += "<td><input type='checkbox' id='selectall' onclick='doCheckboxes();'></td></tr>";
				allrecords += "<td><Select id='selectall' onchange='doCheckboxes(this.id);'><option>--Select Options--</option><option>Select/Unselect All</option><option>1 - 200</option><option>201 - 400</option><option>401 - 600</option><option>601 - 800</option><option>801 - 1000</option><option>1001 - 1200</option><option>1201 - 1400</option><option>1401 - 1600</option><option>1601 - 1800</option><option>1801 - 2000</option></select></td></tr>";
                ...
                //allrecords += "<td>Phone_No</td><td align='center'>Select_All<br><input type='checkbox' id='selectall' onclick='doCheckboxes();'></td></tr>";
                allrecords += "<td>Phone_No</td><td align='center'>Select_All<br><Select id='selectall' onchange='doCheckboxes(this.id);'><option>--Select Options--</option><option>Select/Unselect All</option><option>1 - 200</option><option>201 - 400</option><option>401 - 600</option><option>601 - 800</option><option>801 - 1000</option><option>1001 - 1200</option><option>1201 - 1400</option><option>1401 - 1600</option><option>1601 - 1800</option><option>1801 - 2000</option></select></td></tr>";
            }else if(break_resp[0]=="regularstudentsB"){
                allrecords += "<td width='5%'>S/No</td><td>Matric_No</td><td>Names</td><td>Email</td>";
                //allrecords += "<td>Phone_No</td><td align='center'>Select_All<br><input type='checkbox' id='selectall' onclick='doCheckboxes();'></td></tr>";
                allrecords += "<td>Phone_No</td><td>Pin_No</td><td align='center'>Select_All<br><Select id='selectall' onchange='doCheckboxes(this.id);'><option>--Select Options--</option><option>Select/Unselect All</option><option>1 - 200</option><option>201 - 400</option><option>401 - 600</option><option>601 - 800</option><option>801 - 1000</option><option>1001 - 1200</option><option>1201 - 1400</option><option>1401 - 1600</option><option>1601 - 1800</option><option>1801 - 2000</option></select></td></tr>";
			}
            var recordlist = null;
            ...
            if(break_resp[0]=="regularstudentsB") recordlist = document.getElementById('studentslist');
            if(break_resp[0]=="signatoriestable") recordlist = document.getElementById('signatorylist');
            if(break_resp[0]=="uniquereferencenumbers") recordlist = document.getElementById('pinslist');

            var counter = 0;
            var rsp = "";
            var flg = 0;
            var break_row = "";
            var compare1 = "regularstudents regularstudentsA regularstudentsB studentfees studentpays signatoriestable uniquereferencenumbers ";
            var compare2 = "regularstudents regularstudentsA regularstudentsB studentfees studentpays uniquereferencenumbers ";
            var compare3 = "regularstudents regularstudentsA regularstudentsB studentfees studentpays uniquereferencenumbers ";
            var compare4 = "regularstudents studentfees regularstudentsB uniquereferencenumbers ";
            var compare5 = "regularstudents studentfees uniquereferencenumbers ";
            var compare6 = "regularstudents uniquereferencenumbers ";
            ...
            for(var i=1; i < (break_resp.length-1); i++){
				...
				if(compare3.match(break_resp[0])){
					if(break_resp[0]=="regularstudentsA"){
						...
						myCheckboxes++;
						rsp += "<td align='center'><input type='checkbox' id='"+checkboxid+"'><input type='hidden' id='"+hiddenid+"' value='"+break_row[1]+"'>";
				...
				if(compare9.match(break_resp[0])){
					if(break_resp[0]=="regularstudents"){
						rsp += "<td align='center'>" + break_row[59] + "</td>";
					}else{
				...
				if(compare10.match(break_resp[0])){
					if(break_resp[0]=="regularstudents"){
						...
						var hiddenid="hidden"+(i-1);
						myCheckboxes++;
						rsp += "<td align='center'><input type='checkbox' id='"+checkboxid+"'><input type='hidden' id='"+hiddenid+"' value='"+break_row[1]+"'></td>";
					}else{
...
//						break_row[8] = break_row[8].substr(8,2)+'/'+break_row[8].substr(5,2)+'/'+break_row[8].substr(0,4);
			...
			if(break_resp[0]=="regularstudents"){
				...
				if(readCookie("myregularstudents")=='1'){
					eraseCookie("myregularstudents");
...
//return true;
            ...
            if(navigator.appName == "Microsoft Internet Explorer"){
                inner_codeslist = "<table border='1' style='border-color:#fff;border-style:solid;border-width:1px; height:10px;width:40%;background-color:#336699;margin-top:5px;'>";
            }else{
        ...
        if(resp.match("updateGeneral")){
			...
			if(resp.match("periodexists")){
				...
				$('#showPrompt').dialog('open');
				return true;
			}else if(resp.match("examexists")){
	            break_resp = resp.split("examexists");
				var studentslist = "";
				for(k=1; k<break_resp.length; k++) studentslist += (++sno)+". "+break_resp[k] + "<br>";
				var str="The Session, Semester and Level have been used to update Examinations table for the following students:<br><br>"+studentslist;
				document.getElementById("showPrompt").innerHTML = "<b>Deregistration Not Allowed!!!</b><br><br>"+str;
				$('#showPrompt').dialog('open');
				return true;
			}else if(resp.match("periodused")){
	            break_resp = resp.split("periodused");
				var studentslist = "";
				for(k=1; k<break_resp.length; k++) studentslist += (++sno)+". "+break_resp[k] + "<br>";
				var str="The Session or Level have been used for the following students in the period displayed against them:<br><br>"+studentslist;
				document.getElementById("showPrompt").innerHTML = "<b>Session/Level Used!!!</b><br><br>"+str;
				$('#showPrompt').dialog('open');
				...
				$('#showPrompt').dialog('open');
				//document.getElementById("studentlevels").value = document.getElementById("levels2a").value;
				//document.getElementById("sesionsA").value = document.getElementById("sessions2a").value;
				//document.getElementById("semestersA").value = document.getElementById("semesters2a").value;
				...
				$('#showPrompt').dialog('open');
			}else if(resp.match("studentsregistered")){
				var sessions = document.getElementById("sesionsA").value;
				var semesters = document.getElementById("semestersA").value;
	            break_resp = resp.split("studentsregistered");
				var studentslist = "";
				for(k=1; k<break_resp.length; k++) studentslist += (++sno)+". "+break_resp[k] + "<br>";
				var str="The following students have been Registered for "+sessions+" Session, "+semesters+" Semester:<br><br>"+studentslist;
				document.getElementById("showPrompt").innerHTML = "<b>Students Registereded!!!</b><br><br>"+str;
				$('#showPrompt').dialog('open');
			}else if(resp.match("studentsderegistered")){
				var sessions = document.getElementById("sesionsA").value;
				var semesters = document.getElementById("semestersA").value;
	            break_resp = resp.split("studentsderegistered");
				var studentslist = "";
				for(k=1; k<break_resp.length; k++) studentslist += (++sno)+". "+break_resp[k] + "<br>";
				var str="The following students have been Deregistered for "+sessions+" Session, "+semesters+" Semester:<br><br>"+studentslist;
				document.getElementById("showPrompt").innerHTML = "<b>Students Deregistered!!!</b><br><br>"+str;
				$('#showPrompt').dialog('open');
			}else{
			}
			getRecords("filterbutton",1);
        ...
        if(resp.match("readCookies")){
			...
			var currentrecords = "";
			if(resp.match("readCookies1")) currentrecords = resp.split("readCookies1");
			if(resp.match("readCookies2")) currentrecords = resp.split("readCookies2");
			if(resp.match("readCookies3")) currentrecords = resp.split("readCookies3");
			currentrecord=currentrecords[1].replace(/spc/g, ' ');
			currentrecord=currentrecord.replace(/coma/g, ',');
			var break_resp = currentrecord.split("_-_");
			var response = "<b>Upload in Progress!!!</b><br><br>Your file upload is in progress.........<br><br><br>";
			if(resp.match("readCookies1") || resp.match("readCookies2")){
				response += "Start Time:			"+starttime+"<br><br>";
				response += "Current Time:			"+t+"<br><br>";
				response += "Current Sheet:			"+break_resp[0]+"<br><br>";
				response += "Current Row:			"+break_resp[1]+"<br><br>";
				if(resp.match("readCookies1")){
					response += "Current Course Code:	"+break_resp[2]+"<br><br>";
					response += "Current Matric No:		"+break_resp[3]+"<br><br>";
					response += "Current Name:			"+break_resp[4];
				}
				if(resp.match("readCookies2")){
					response += "Current Matric No:		"+break_resp[2]+"<br><br>";
					response += "Current Last Name:		"+break_resp[3]+"<br><br>";
					response += "Current Other Names:	"+break_resp[4];
				}
			}
			if(resp.match("readCookies3")){
				if(break_resp[1]=="stop"){
					myStopFunction();
					response = "<b>Result Processing in Finished!!!</b><br><br>Your result processing has finished,<br><br>Click OK to close this box. <br><br><br>";
					response += "Start Time:			"+starttime+"<br><br>";
					response += "Current Time:			"+t+"<br><br>";
					document.getElementById("showRecord").innerHTML = response;
					$('#showRecord').dialog('open');
					//$('#showRecord').dialog('close');
					return true;
				}
				response = "<b>Result Processing in Progress!!!</b><br><br>Your result processing is in progress.........<br><br><br>";
				response += "Start Time:			"+starttime+"<br><br>";
				response += "Current Time:			"+t+"<br><br>";
				response += "S/No:	            "+break_resp[0]+"<br><br>";
				response += "Current Matric No:	"+break_resp[1]+"<br><br>";
				response += "Current Name:		"+break_resp[2];
			}
			document.getElementById("showRecord").innerHTML = response;
			$('#showRecord').dialog('open');
		}
...
//$('#showPrompt').dialog('open');
            ...
            if(break_resp[0]=="regularstudents"){
				...
				if(break_resp[16]!=null && break_resp[16]!=""){
                ...
                document.getElementById("carryover").value = break_resp[55];
                document.getElementById("sesionsreabsurption").value = break_resp[56];
				populateStudentHistory(break_resp[57],break_resp[2]);
                if(break_resp[57]!=null) document.getElementById("sessionss").value = break_resp[58];
                if(break_resp[58]!=null) document.getElementById("semesterss").value = break_resp[59];
				break_reg = break_resp[60].split("!!!");
				//var str = "<table style='border-color:#fff;border-style:solid;border-width:1px; height:10px;width:100%;background-color:#336699;margin-top:5px;'>";
				var str = "<table style='border-color:#fff;border-style:solid;border-width:1px; height:10px;width:100%;background-color:#336699;margin-top:5px;'>";
				str += "<tr style='font-weight:bold; color:white'><td colspan='4' align='center'>REGISTERED LEVELS, SESSIONS/SEMESTERS</td></tr>";
				str += "<tr style='font-weight:bold; color:white'>";
				str += "<td>S/No</td><td>LEVEL</td><td>SESSION</td><td width='15%'>SEMESTER</td><td>REGISTERED</td></tr>";
				flag=0;count=0;
				for(var k=0; k<(break_reg.length-1); k++){
					...
					str += "<td align='right'>"+(++count)+".</td>"+"<td>"+break_col[0]+"</td>"+"<td>"+break_col[1]+"</td>"+"<td>"+break_col[2]+"</td>"+"<td>"+break_col[3]+"</td></tr>";
				}
				str += "</table>";
            ...
            if(break_resp[0]=="amendedreasons"){
                document.getElementById("amendreason").value= break_resp[1];
            }
        }
		...
		if(resp.match("already passed")){
			populateMark('a.regNumber','DESC');
			var resp = resp.replace(/_/g, ' ');
			resp = resp.replace(/examresultstableinserted/g, ' ');
			resp = resp.replace(/examresultstableupdated/g, ' ');
			document.getElementById("showError").innerHTML = "<b>Alert!!!</b><br><br>"+resp;
			$('#showError').dialog('open');
			return true;
		}

		if(resp.match("updateStudentHistory")){
			break_resp = resp.split("updateStudentHistory");
			break_row = resp.split("~_~");
			populateStudentHistory(break_row[1],break_row[2]);
		}
...
//$('#showPrompt').dialog('open');
        ...
        if(resp.match("deleted")){
            ...
            if(break_resp[0]=='examresultstable' || break_resp[0]=='retakecourses' || break_resp[0]=='specialfeatures') {
                //populateMark(readCookie("sortby"));
            document.getElementById("showPrompt").innerHTML = readCookie("mystr");
            }else if(break_resp[0]=='signatoriestable'){
				//resetForm();
				document.getElementById("signatoryposition").value="";
				document.getElementById("signatoryname").value="";
        ...
        if(resp.match("recordlocked")){
            ...
            document.getElementById("showPrompt").innerHTML = "<b>Records Locked!!!</b><br><br>The records you are trying to access are locked "+break_resp[1];
			$('#showPrompt').dialog('open');
        }

        if(resp.match("uploadlocked")){
            break_resp = resp.split("uploadlocked");
            document.getElementById("showPrompt").innerHTML = "<b>Records Locked!!!</b><br><br>The records you are trying to upload are locked "+break_resp[1];
			$('#showPrompt').dialog('open');
        }
        if(resp.match("uploadopen")){
			alert("Please ensure that:\n\n\n 1. Only Excel file types may be selected.\n 2. Only Excel files with .xls extension i.e. Excel 97 - Excel 2003 versions are acceptable.\n\n\n\nPlease click ok to continue");
        ...
        if(resp.match("noemailaddress")){
			document.getElementById('f1_upload_process').style.visibility = 'hidden';
            document.getElementById("showPrompt").innerHTML = "No email was setup for the students";
            $('#showAlert').dialog('close');
            $('#showPrompt').dialog('open');
		}

        /*if(resp.match("connectionerror")){
			break_resp = resp.split("connectionerror");
			document.getElementById('f1_upload_process').style.visibility = 'hidden';
            document.getElementById("showPrompt").innerHTML = "Server returned the following error:<br><br>"+break_resp+"<br><br>Connection to server returned error";
            $('#showAlert').dialog('close');
            $('#showPrompt').dialog('open');
		}*/
        ...
        if(resp.match("connectionerror")){
			...
			break_resp = resp.split("connectionerror");
            $('#showAlert').dialog('close');
            document.getElementById("showPrompt").innerHTML = "<h3>Transcript Email Response!</h3><br><br><b>Your attempt to send transcripts by email failed.<br><br>Connection to server returned error.";
            $('#showPrompt').dialog('open');
            return true;
        }
        if(resp.match("emailsuccessfully")){
			document.getElementById('f1_upload_process').style.visibility = 'hidden';
            document.getElementById("showPrompt").innerHTML = "Statement of results were successfully sent to students";
            $('#showAlert').dialog('close');
            $('#showPrompt').dialog('open');
		}
        ...
        if(resp.match("coursenotsetup")){
            ...
            document.getElementById("showError").innerHTML = "<b>Course Code Not Setup!!!</b><br><br> "+break_resp[1]+" not setup for: <br><b>School - </b>"+break_resp[2]+"<br><b>Department - </b>"+break_resp[3]+"<br><b>Programme - </b>"+break_resp[4]+"<br><b>Level - </b>"+break_resp[5]+"<br><b>Session - </b>"+break_resp[6]+"<br><b>Semester - </b>"+break_resp[7]; //+"<br><b>Group Code - </b>"+break_resp[8];
			$('#showError').dialog('open');
        }
        ...
        if(resp.match("exceedmaximumunits")){
            resp = resp.replace(/_/g, ' ');
        ...
        if(resp.match("regupdated")){
			var oldregnumber=document.getElementById("oldregNumber").value;
			var newregnumber=document.getElementById("newregNumber").value;
			document.getElementById("regNumber").value=newregnumber;
            document.getElementById("showPrompt").innerHTML = "<b>Matric No Updated!!!</b><br><br>All "+oldregnumber+" have been successfully replaced with "+newregnumber+" in the entire database.";
            $('#showPrompt').dialog('open');
        }

        if(resp.match("regexists")){
			var newregnumber=document.getElementById("newregNumber").value;
            document.getElementById("showPrompt").innerHTML = "<b>Matric No Exists!!!</b><br><br>The new Matric No "+newregnumber+" already exists for a student in the database.";
            $('#showPrompt').dialog('open');
        }

        if(resp.match("regblank")){
			var newregnumber=document.getElementById("newregNumber").value;
            document.getElementById("showPrompt").innerHTML = "<b>Blank Matric No!!!</b><br><br>The new Matric No "+newregnumber+" is blank and can not be updated in the database.";
            $('#showPrompt').dialog('open');
        }

        if(resp.match("pingeneratesuccess")){
			//getRecords('listpins');
            document.getElementById("showPrompt").innerHTML = "<b>Pin Numbers Generation Successful!!!</b><br><br>Generation of Pin Numbers is successful, <br>Click [List Pins Numbers] button to display the generated pins.";
            $('#showPrompt').dialog('open');
        }

        if(resp.match("pinsavailable")){
			//getRecords('listpins');
            document.getElementById("showPrompt").innerHTML = "<b>Pin Numbers Not Generated!!!</b><br><br>No new Pin Numbers are Generated, <br>The existing ones are not yet exhausted, <br>Click [List Pins Numbers] button to display the existing pins.";
            $('#showPrompt').dialog('open');
        }

        if(resp.match("pinallocationsuccess")){
			//getRecords('list4pins');
            document.getElementById("showPrompt").innerHTML = "<b>Pin Numbers Allocation Successful!!!</b><br><br>Pin Numbers are successfully allocated, <br>Click [List Students] button again to display the allocated pins.";
            $('#showPrompt').dialog('open');
        }

        if(resp.match("pinallocationfailed")){
			//getRecords('list4pins');
            document.getElementById("showPrompt").innerHTML = "<b>Pin Numbers Allocation Failed!!!</b><br><br>No Pin Numbers are allocated.";
            $('#showPrompt').dialog('open');
        }

        if(resp.match("onlineuploadsuccess")){
            document.getElementById("showPrompt").innerHTML = "<b>Online Upload Successful!!!</b><br><br>Results online upload is successful.";
            $('#showPrompt').dialog('open');
        }
